...
extern "C" int run_hopper_f16_moe_sw_interleaved_presum_pingpong_max_fusion_tma_reduce_4x128(
	KernelRunnerBuffers, int, int, int, int, int, cudaStream_t *, int, int, float *);

class F16HopperMoePingpongTmaReduce4x128Test
	: public testing::TestWithParam<strassen_tests::MoeTestCase> {};

TEST_P(F16HopperMoePingpongTmaReduce4x128Test, MatchesReference) {
	strassen_tests::run_moe_gtest_case(
		GetParam(), run_hopper_f16_moe_sw_interleaved_presum_pingpong_max_fusion_tma_reduce_4x128);
}

INSTANTIATE_TEST_SUITE_P(
	,
	F16HopperMoePingpongTmaReduce4x128Test,
