...
extern "C" int run_hopper_f16_moe_sw_interleaved_presum_cooperative_max_fusion_2x256(
	KernelRunnerBuffers, int, int, int, int, int, cudaStream_t *, int, int, float *);

class F16HopperMoeCooperative2x256Test
	: public testing::TestWithParam<strassen_tests::MoeTestCase> {};

TEST_P(F16HopperMoeCooperative2x256Test, MatchesReference) {
	strassen_tests::run_moe_gtest_case(
		GetParam(), run_hopper_f16_moe_sw_interleaved_presum_cooperative_max_fusion_2x256);
}

INSTANTIATE_TEST_SUITE_P(
	,
	F16HopperMoeCooperative2x256Test,
